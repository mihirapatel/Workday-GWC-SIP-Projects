...
one = random.randint(0,700)
two = random.randint(0,500)
three = random.randint(-10, 10)
four = random.randint(-10, 10)
five = random.randint(10, 30)
...
class BouncingBall(): 
    ...
    def flashBounce(self, screen, colors, screen_width, screen_height):
    	...
    	ball_color = random.choice(ballColors) # This is outside because of variable scoping.
...
ballColors = [BLACK, WHITE, GREEN, RED, BLUE, GREY]

ball1 = BouncingBall(one, two, three, four, five)
ball2 = BouncingBall(one, two, three, four, five)
...
while not done:
    ...
    ball1.flashBounce(screen, ballColors, SCREEN_WIDTH, SCREEN_HEIGHT)
    ball2.flashBounce(screen, ballColors, SCREEN_WIDTH, SCREEN_HEIGHT)
